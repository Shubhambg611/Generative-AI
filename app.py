...
def create_vector_embedding():
    if "vectors" not in st.session_state:
        ...
        st.session_state.final_document = st.session_state = st.session_state.text_splitter.split_documents(st.session_state.docs[:50])
        st.session_state.vectors = FAISS.from_documents(st.session_state.final_document,st.session_state.embeddings)

user_prompt = st.text_input("Input your query from the research paper")
